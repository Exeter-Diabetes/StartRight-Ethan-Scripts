...
my_data = my_data %>%
  mutate(Duration_diabetes = as.numeric(
    difftime(
...
# Clin F + GRS Model = 24.46138054 + (-4.443506884 * Log(Age)) + (-5.534741384 *Log(BMI)) + (33.93968* T1D GRS)
# CLin F + Lipids = 9.0034272  - (0.1915482*BMI) – (0.1686227*age at diagnosis) + (0.3026012 if female) – (0.2269216*cholesterol) + (1.540850*HDL) – (0.2784059*triglycerides)
...
my_data = my_data %>% # Sixth Model: ClinF + GRS
  mutate( logOR_clinF_GRS = 24.46138054 
...
my_data = my_data %>%
  mutate( logOR_clinF_Lipids = 9.0034272
          - (0.1915482*BMI) 
...
#> Creating Binary outcome if patients were atleast 1 antibody positive
Type2_cohort = Type2_cohort %>%
...
Type2_cohort %>%
  select(Insulin, V2Insulin, V3Insulin, Time_to_insulin, Date_continuous_insulin, progressed)
...
table = print(
  CreateTableOne(
    ...
    vars = c("BMI", "AgeatDiagnosis", "GRS", "HbA1c_at_diagnosis", "model_clinF",
             ...
             "model_clinF_AntiGRS", "Number_Antibodies", "Duration_diabetes"),
    strata = "progressed"
  ),
  test = FALSE,
  noSpaces = TRUE
...
#> REMOVED: & !is.na(model_clinF_GAD) & !is.na(model_clinF_IA2) & !is.na(model_clinF_GRS) & !is.na(model_clinF_Lipids) )
...
for (x in 1:10){
  ...
  if (x == 1) {
    ...
    total = Type2_cohort %>%
      filter(model_clinF_AntiGRS <= 0.1) %>%
      nrow()
    
    antibody = Type2_cohort %>%
      filter(model_clinF_AntiGRS <= 0.1 & progressed == 1) %>%
      nrow()
    ...
    total = Type2_cohort %>%
      filter(model_clinF_AntiGRS > 0.9) %>%
      nrow()
    
    antibody = Type2_cohort %>%
      filter(model_clinF_AntiGRS > 0.9 & progressed == 1) %>%
      nrow()
    ...
    total = Type2_cohort %>%
      filter(model_clinF_AntiGRS > deciles[x-1] & model_clinF_AntiGRS <= deciles[x]) %>%
      nrow()
    
    antibody = Type2_cohort %>%
      filter(model_clinF_AntiGRS > deciles[x-1] & model_clinF_AntiGRS <= deciles[x] & progressed == 1) %>%
      nrow()
...
plot(deciles, results,
     xlim = c(0,1),
     ylim = c(0,1),
     main = 'Clinical Features + Antibodies + GRS Model',
     xlab = 'Clin F + Anti + GRS Model Probability',
     ylab = '% Progressed to Insulin <= 3 years')

#For r-squared value
summary(lm(unlist(results) ~ unlist(deciles), data=Type2_cohort))

#graph trendline and print equation
calibration_plot = lm(unlist(results) ~ unlist(deciles), data=Type2_cohort)
lines(unlist(deciles),
      predict(calibration_plot),
      col = 2,
      lwd = 2)
abline(a=0, b=1) # Adding a y=x line
...
``` {r Clinical Features Model Discerns Antibody Positivity}
#> This Section used to generate Graphs showing how Clin F Model discriminates antibody positive patients
Type2_cohort$Antibody_Positive

#Distribution 2.0: Boxplot
ggplot(Type2_cohort,aes(as.factor(Number_Antibodies), model_clinF)) +
  geom_boxplot() +
...
#> Following Section tests the Clin F + GRS Model to see if it works :D
ggplot(Type2_cohort,aes(as.factor(Antibody_Positive), model_clinF_GRS)) +
  geom_boxplot() +
  xlab("Antibody Positivity") +
  ylab("Clinical Features Model Probabilities") +
  ylim(c(0,1))
...
# ROC AUC
glm.fit = glm(roc_data$Antibody_Positive ~ roc_data$model_clinF_GRS, family = binomial())
plot.new()
lines(roc_data$model_clinF_GRS, glm.fit$fitted.values)
