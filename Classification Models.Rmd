...
#To Do: REMOVE ZNT8 Code Properly
my_data=my_data %>%
  mutate(ZNT_status = 0)
...
# Type2_cohort %>%
#     filter(progressed == 1) %>%
#     select(V3Date_Contacted, V2Date_Contacted, DateofDiagnosis)
my_data = my_data %>%
...
#> ggBarPlot for antibody distribution - NOT USED IN PRESENTATION
# ggplot(
#   Type2_cohort %>%
...
``` {r Clin F + Antibodies Model Adds to antibodies interpretation}
#> This section is dedicated to the final slide, which shows Clin F + Anti Model adds discrimination within antibody +ve cohort

#> To use this section:
#>  - Change model_probabilities to the specified model (either "model_clinF_Anti" OR "model_clinF_AntiGRS") 
#>  - Change model probabilities on line 507 to produce associated box plot
...
# n for Within Antibody-positive group
nrow(temp_roc_data)

# Reassign model_probabilities
model_probabilities = temp_roc_data$model_clinF_AntiGRS
